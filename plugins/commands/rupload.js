...
module.exports = {
    ...
    onRun: async function({ api, event, args }) {
        ...
        try {
            ...
            const attachment = replyInfo.attachments[0];
            const detectedType = attachment.type === 'animated_image'
                ? 'gif'
                : (attachment.type === 'photo' ? 'image' : (attachment.type || 'image'));
            const mediaType = (args[0] || detectedType || 'image').toLowerCase();
            
            if (!['image', 'video', 'audio', 'gif', 'photo', 'animated_image'].includes(attachment.type)) {
                return api.sendMessage(
            ...
            if (!mediaUrl) {
                return api.sendMessage(
                    "> Không tìm thấy URL media để upload",
                    threadID,
                    null,
                    messageID
                );
            ...
            const ext = mediaType === 'video' ? 'mp4' : mediaType === 'audio' ? 'mp3' : mediaType === 'gif' ? 'gif' : 'jpg';
